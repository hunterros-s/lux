...
use crate::types::{Action, Plugin, Source, Trigger, View};
...
impl PluginRegistry {
    ...
    pub fn get_root_sources(&self) -> Vec<(String, usize)> {
        ...
        tracing::debug!(
            "get_root_sources: {} plugins, {} sources registered",
            plugins.len(),
            sources.len()
        );
        ...
        tracing::debug!("get_root_sources: found {} root sources", result.len());
        result
    }
